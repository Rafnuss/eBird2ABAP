...
%% Filter protocol
ebd.KEEP_PROTOCOL = ismember(ebd.PROTOCOLTYPE, ["Historical", "Incidental", "Stationary", "Traveling"]);
% mean(ebd.KEEP_PROTOCOL) 99%
ebd.KEEP_PROTOCOL(ebd.PROTOCOLTYPE=="Historical" & isnan(ebd.EFFORTDISTANCEKM)) = false;
% mean(ebd.KEEP_PROTOCOL) 88%
ebd.KEEP_PROTOCOL(~ebd.ALLSPECIESREPORTED) = false;
...
% mean(ebd.KEEP_PENTAD) % 66% -> 737% for 1.2 overlap
...
id = ebd.KEEP_PENTAD & ebd.KEEP_PROTOCOL & ebd.DURATIONMINUTES>0;
% mean(id) 23%
...
for i_card = 1:height(card)
    ...
    id = ebd.PENTAD == card.pentad(i_card) & ebd.OBSERVERID == card.observer(i_card) & ebd.OBSERVATIONDATE_num >= card.date(i_card) & ebd.OBSERVATIONDATE_num < card.date(i_card)+5;
...
% sum(ebd.KEEP_PENTAD&ebd.KEEP_PROTOCOL)  19K
% sum(ebd.card~="") 10K 
...
ebd0f = ebd0(ismember(ebd0.SAMPLINGEVENTIDENTIFIER, all_checklists),["TIMEOBSERVATIONSSTARTED", "SAMPLINGEVENTIDENTIFIER", "SCIENTIFICNAME", "GLOBALUNIQUEIDENTIFIER"]);


for i_card = 1:height(card)
    id2 = ismember(ebd0f.SAMPLINGEVENTIDENTIFIER, d{i_card}.Checklists);
    d{i_card}.StartTime = min(ebd0f.TIMEOBSERVATIONSSTARTED(id2));
    d{i_card}.Species = unique(ebd0f.SCIENTIFICNAME(id2));
    d{i_card}.TotalSpp = numel(d{i_card}.Species);
end

fid = fopen('export/KE_data.json','w');
